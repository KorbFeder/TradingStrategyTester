...
export class FrontTesting {
    ...
    private async useStrategy(data: OHLCV[], symbol: string, strategy: IStrategy) {
        ...
        if(!(tradeDirection == TradeDirection.HOLD)) {
            if(!strategy.usesDynamicExit) {
				...
				if(await this.position(symbol, sl.stop, sl.target, tradeDirection)) {
	...
	private async position(symbol: string, stopLoss: number, target: number, tradeDirection: TradeDirection): Promise<boolean> {
		...
		let size = PositionSize.calculate(risk, balance.free, currentPrice, stopLoss);

		if(TradeDirection.SELL == tradeDirection) {
			size = -size;
		}

		await this.db.saveOrder(tradeDirection == TradeDirection.BUY ? 'long' : 'short', symbol, size, stopLoss, target, currentPrice);
